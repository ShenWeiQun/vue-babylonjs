  ...
  md-layout.issues-card
    ...
    md-whiteframe
      md-tabs(@change="changeType")
        md-tab(:md-label="type.label" v-for="(type,pIndex) in fTypes" :key="pIndex")
          form
            md-input-container(v-for="(item,cIndex) in type.items" :key="cIndex")
...
export default {
  ...
  methods: {
    ...
    openIssue() {
      let isValid = true;
      const {
        issueType, content, fTypes, formData, openDialog,
      } = this;
      const data = formData[fTypes[issueType].name];
  ...
  computed: {
    content() {
      const lines = [];
      const { fTypes, issueType, formData } = this;
      const formType = fTypes[issueType];
